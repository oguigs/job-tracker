...
def carregar_perfil_empresa(nome: str):
    con = conectar()
    empresa = con.execute("SELECT id, nome, ramo, cidade, estado, url_vagas, url_site_oficial, favicon_url, data_cadastro FROM dim_empresa WHERE nome = ?", [nome]).df()
    vagas = con.execute("SELECT id, titulo, nivel, modalidade, stacks, link, data_coleta, ativa, candidatura_status FROM fact_vaga WHERE id_empresa = (SELECT id FROM dim_empresa WHERE nome = ?) AND (negada = false OR negada IS NULL) ORDER BY data_coleta DESC", [nome]).df()
    logs = con.execute("SELECT vagas_encontradas, vagas_novas, status, data_execucao FROM log_coleta WHERE empresa = ? ORDER BY data_execucao DESC LIMIT 5", [nome]).df()
    enderecos = con.execute("SELECT cidade, bairro FROM dim_empresa_endereco WHERE id_empresa = (SELECT id FROM dim_empresa WHERE nome = ?)", [nome]).fetchall()
    con.close()
