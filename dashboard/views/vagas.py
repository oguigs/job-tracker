...
def _limpar_filtros():
    for k in ["f_empresa", "f_nivel", "f_modalidade", "f_status_cand",
              "f_status_vaga", "f_sla_dias", "f_busca",
              "f_ordenar_por", "f_num_colunas", "f_modo_compacto",
              "filtro_rapido_vagas"]:
...
def render():
    ...
    sla_dias = st.sidebar.number_input(
        "Sem resposta há (dias)", min_value=0, max_value=30, value=0, step=1, key="f_sla_dias",
    ...
    df_base = df.copy()
    if empresa_sel   != "Todas":  df_base = df_base[df_base["empresa"]   == empresa_sel]
    if nivel_sel     != "Todos":  df_base = df_base[df_base["nivel"]     == nivel_sel]
    if modalidade_sel!= "Todas":  df_base = df_base[df_base["modalidade"]== modalidade_sel]
    if status_vaga   == "Ativas": df_base = df_base[df_base["ativa"]     == True]
    elif status_vaga == "Encerradas": df_base = df_base[df_base["ativa"] == False]
    if busca: df_base = df_base[df_base["titulo"].str.contains(busca, case=False, na=False)]
    if sla_dias > 0:
        limite = (datetime.now() - timedelta(days=sla_dias)).strftime("%Y-%m-%d")
        df_base = df_base[(df_base["candidatura_status"] == "inscrito") &
    ...
    n_processo   = df_base[df_base["candidatura_status"].isin(
        ["chamado", "recrutador", "fase_1", "fase_2", "fase_3"])].shape[0]

    filtro_rapido = st.session_state["filtro_rapido_vagas"]
    col1, col2, col3, col4 = st.columns(4)
    if col1.button(f"**Todas**  \n{n_total}", use_container_width=True,
                   type="primary" if filtro_rapido is None else "secondary"):
        st.session_state["filtro_rapido_vagas"] = None; st.rerun()
    if col2.button(f"**Não inscritas**  \n{n_nao_inscrito}", use_container_width=True,
                   type="primary" if filtro_rapido == "nao_inscrito" else "secondary"):
        st.session_state["filtro_rapido_vagas"] = "nao_inscrito"; st.rerun()
    if col3.button(f"**Inscritas**  \n{n_inscrito}", use_container_width=True,
                   type="primary" if filtro_rapido == "inscrito" else "secondary"):
        st.session_state["filtro_rapido_vagas"] = "inscrito"; st.rerun()
    if col4.button(f"**Em processo**  \n{n_processo}", use_container_width=True,
                   type="primary" if filtro_rapido == "em_processo" else "secondary"):
        st.session_state["filtro_rapido_vagas"] = "em_processo"; st.rerun()
    ...
    if df_f.empty:
        filtros_ativos = any([
            empresa_sel != "Todas", nivel_sel != "Todos",
            modalidade_sel != "Todas", status_vaga != "Ativas",
            busca, sla_dias > 0, filtro_rapido is not None
        ])
        if filtros_ativos:
            render_empty_state(
    ...
    # ── ORDENAÇÃO ──────────────────────────────────────────────
    if ordenar_por == "Score ↓":   df_f = df_f.sort_values("score", ascending=False)
    elif ordenar_por == "Score ↑": df_f = df_f.sort_values("score", ascending=True)
    elif ordenar_por == "Data ↓":  df_f = df_f.sort_values("data_coleta", ascending=False)
    elif ordenar_por == "Data ↑":  df_f = df_f.sort_values("data_coleta", ascending=True)
    elif ordenar_por == "Empresa A-Z": df_f = df_f.sort_values("empresa", ascending=True)
    ...
    if modo_compacto:
        for _, vaga in df_f.iterrows():
            ...
            if status_cand_val == "nao_inscrito":
                ...
                c5.markdown(f"<span style='color:#1D9E75;font-size:11px'>{label_status}</span>",
