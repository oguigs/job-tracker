...
import pandas as pd
import json
from dashboard.components import conectar, extrair_stacks_flat, grafico_stacks
...
def render():
    ...
    nomes = [e[0] for e in empresas_db]
    ...
    emp_b = col2.selectbox("Empresa B", nomes, index=1 if len(nomes) > 1 else 0)
    ...
    with col_a:
        ...
        nivel_a = df_a["nivel"].mode()[0] if not df_a.empty else "—"
        m2.metric("Nível predominante", nivel_a)
        modal_a = df_a["modalidade"].mode()[0] if not df_a.empty else "—"
        m3.metric("Modalidade predominante", modal_a)

    with col_b:
        st.subheader(emp_b)
        m1, m2, m3 = st.columns(3)
        m1.metric("Vagas", len(df_b))
        nivel_b = df_b["nivel"].mode()[0] if not df_b.empty else "—"
        m2.metric("Nível predominante", nivel_b)
        modal_b = df_b["modalidade"].mode()[0] if not df_b.empty else "—"
        m3.metric("Modalidade predominante", modal_b)
    ...
    with col_a:
        if not df_a.empty:
            df_nivel = df_a["nivel"].value_counts().reset_index()
            df_nivel.columns = ["nivel", "count"]
            fig = px.pie(df_nivel, values="count", names="nivel",
                title=emp_a, color_discrete_sequence=px.colors.qualitative.Set2,
    ...
    with col_b:
        if not df_b.empty:
            df_nivel = df_b["nivel"].value_counts().reset_index()
            df_nivel.columns = ["nivel", "count"]
            fig = px.pie(df_nivel, values="count", names="nivel",
                title=emp_b, color_discrete_sequence=px.colors.qualitative.Set2,
                template="plotly_white")
            fig.update_layout(height=280, margin=dict(l=0,r=0,t=40,b=0))
            st.plotly_chart(fig, use_container_width=True)