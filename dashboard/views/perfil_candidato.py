...
from database.candidato import (
    ...
    carregar_stacks,
    deletar_stack,
    salvar_curriculo_texto,
    carregar_curriculo_texto,
)
...
def render():
    ...
    with tab_dados:
        with st.form("form_perfil"):
            ...
            linkedin = col3.text_input(
                "LinkedIn",
                value=p.get("linkedin") or "",
            ...
            modalidade_pref = col6.selectbox(
                "Modalidade preferida",
                MODALIDADES,
                index=MODALIDADES.index(p["modalidade_pref"])
                if p.get("modalidade_pref") in MODALIDADES
                else 0,
            )

            col7, col8 = st.columns(2)
            pretensao_min = col7.number_input(
                "Pretensão mínima (R$)",
                min_value=0,
                step=500,
                value=int(p.get("pretensao_min") or 0),
            )
            pretensao_max = col8.number_input(
                "Pretensão máxima (R$)",
                min_value=0,
                step=500,
                value=int(p.get("pretensao_max") or 0),
            )
            ...
            if st.form_submit_button("Salvar perfil", type="primary", use_container_width=True):
                salvar_perfil(
                    ...
                    cidade=cidade,
                    nivel=nivel,
                    modalidade_pref=modalidade_pref,
                    pretensao_min=pretensao_min,
                    pretensao_max=pretensao_max,
                    resumo=resumo,
                )
        ...
        with st.form("form_stack"):
            ...
            if st.form_submit_button("Adicionar", type="primary", use_container_width=True):
                if not stack:
                    ...
                    salvar_stack(
                        id_candidato, stack.lower().strip(), categoria, nivel_stack, anos_exp
                    )
    ...
    with tab_curriculo:
        ...
        st.caption(
            "O texto será salvo e usado automaticamente para calcular scores ATS nas vagas capturadas."
        )
        ...
        if cv_file:
            ...
            try:
                ...
                if texto.strip():
                    ...
                    st.error(
                        "Não foi possível extrair texto do PDF. Use um PDF com texto selecionável."
                    )
            finally:
                os.unlink(tmp_path)
        ...
        if texto_atual:
            ...
            st.caption(
                "Recalcula ANYA para todas as vagas ativas com descrição usando o currículo armazenado."
            )
            if st.button(
                "↻ Recalcular todos os scores", type="secondary", use_container_width=True
            ):
