...
def render():
    ...
    with tab_categoria:
        ...
        if df.empty:
            ...
            top_stacks = (df.groupby("stack")["quantidade"]
            ...
            if len(df_top["data_ref"].unique()) < 2:
                ...
                fig.update_layout(height=400, margin=dict(l=0,r=0,t=40,b=0),
                    yaxis_title="", xaxis_title="Vagas")
            else:
                df_top["data_ref"] = pd.to_datetime(df_top["data_ref"])
                fig = px.line(df_top, x="data_ref", y="quantidade",
                    color="stack", title=f"Top stacks — {categoria_sel}",
                    template="plotly_white",
                    color_discrete_sequence=px.colors.qualitative.Set2)
                fig.update_layout(height=400, margin=dict(l=0,r=0,t=40,b=0),
                    xaxis_title="Data", yaxis_title="Vagas", legend_title="Stack")
    ...
    with tab_stack:
            ...
            if df.empty:
                ...
                if len(df["data_ref"].unique()) < 2:
                    ...
                    fig = px.line(df, x="data_ref", y="quantidade",
                        title=f"Evolução de '{stack_sel}'",
                        template="plotly_white",
                        color_discrete_sequence=[cor])
                    fig.update_layout(height=350, margin=dict(l=0,r=0,t=40,b=0),
                        xaxis_title="Data", yaxis_title="Vagas")
                    st.plotly_chart(fig, use_container_width=True)
