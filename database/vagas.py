...
def inserir_vaga_manual(
    ...
    origem: str,
    contato: str,
    stacks_override: dict | None = None,
    nivel_override: str | None = None,
    modalidade_override: str | None = None,
) -> int:
    from transformers.stack_extractor import extrair_stacks, detectar_nivel, detectar_modalidade

    stacks = stacks_override if stacks_override is not None else extrair_stacks(descricao)
    nivel = nivel_override if nivel_override else detectar_nivel(titulo)
    modalidade = modalidade_override if modalidade_override else detectar_modalidade(descricao)
...
def atualizar_descricao_vaga(
    id_vaga: int, descricao: str, stacks_json: str = None, modalidade: str = None
):
    with db_connect() as con:
        if stacks_json and modalidade:
            ...
            con.execute("UPDATE fact_vaga SET descricao=? WHERE id=?", [descricao, id_vaga])
