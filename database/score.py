...
def calcular_score(id_vaga: int, id_candidato: int) -> dict:
    ...
    vaga_lista = [{"stack": t.lower(), "categoria": cat}
                  for cat, termos in stacks_vaga.items() for t in termos]
    if not vaga_lista:
        return {"score": 0, "breakdown": {}, "matches": [], "gaps": [],
                "total_vaga": 0, "total_match": 0}
    ...
    for item in vaga_lista:
        ...
        if stack in candidato_set:
            matches.append({"stack": stack, "categoria": categoria,
    ...
    return {"score": score, "matches": matches, "gaps": gaps,
            "breakdown": breakdown, "total_vaga": len(vaga_lista),
...
def calcular_scores_todos(id_candidato: int) -> dict:
    with db_connect(read_only=True) as con:
        ...
        """).fetchall()
    return {id_vaga: calcular_score(id_vaga, id_candidato)["score"]
