...
def adicionar_nota(id_vaga: int, nota: str) -> int:
    with db_connect() as con:
        ...
        con.execute("INSERT INTO log_candidatura (id, id_vaga, nota) VALUES (?, ?, ?)",
...
def listar_notas(id_vaga: int):
    with db_connect(read_only=True) as con:
        return con.execute("""
            SELECT id, data_nota, nota FROM log_candidatura
            WHERE id_vaga = ? ORDER BY data_nota DESC, id DESC
        """, [id_vaga]).df()
